...
class ABACDecision(BaseModel):
    """OPA ABAC policy evaluation result."""
    allow: bool
    pii_fields: list[str] = []
...
async def check_abac(user: TokenClaims, resource: ResourceRequest) -> ABACDecision:
    ...
    OPA input schema:
      input.subject.{role, department, clearance, tenant_id, user_id}
...
async def check_permission(
    ...
    """
    Legacy compatibility wrapper — same interface as original.
    Returns {"allow": bool, "filters": list, "pii_fields": list}.
    """
    user = TokenClaims(
        sub=user_id,
        tenant_id=tenant_id,
