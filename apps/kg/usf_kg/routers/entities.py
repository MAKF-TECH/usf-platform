...
# ── Request/response for label-based resolution ───────────────────────────────
...
class ResolveLabelRequest(BaseModel):
    ...
    embedding: list[float] | None = Field(default=None, description="Optional sentence embedding for vector search")
...
async def resolve_entity_by_label(request: Request, body: ResolveLabelRequest):
    ...
    Steps: exact ArcadeDB lookup → vector search (if embedding provided) → mint new IRI.
    """
    resolver = request.app.state.entity_resolver
    try:
