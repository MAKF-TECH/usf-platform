"""001_initial_schema — USF Platform initial database schema.

Creates all core tables in the `usf` PostgreSQL schema.
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint("industry IN ('banking','healthcare','energy','retail','telecom','manufacturing','public_sector','other')", name="ck_tenant_industry"),
        sa.CheckConstraint("slug ~ '^[a-z][a-z0-9-]{2,62}$'", name="ck_tenant_slug"),
        sa.CheckConstraint("plan IN ('trial','starter','professional','enterprise')", name="ck_tenant_plan"),
        schema="usf",
    )
    ...
    op.create_table(
        "user",
        sa.Column("id", PGUUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("tenant_id", PGUUID(as_uuid=True), sa.ForeignKey("usf.tenant.id", ondelete="CASCADE"), nullable=False),
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.CheckConstraint("role IN ('admin','analyst','risk_analyst','finance_analyst','compliance_officer','auditor','viewer','ingestion_operator')", name="ck_user_role"),
        sa.CheckConstraint("clearance_level IN ('public','internal','confidential','restricted','top_secret')", name="ck_user_clearance"),
        schema="usf",
    )
    ...
    op.create_table(
        "refresh_token",
        sa.Column("id", PGUUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("user_id", PGUUID(as_uuid=True), sa.ForeignKey("usf.user.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_table(
        ...
        sa.CheckConstraint("type IN ('warehouse','file','api','stream')", name="ck_data_source_type"),
        sa.CheckConstraint("status IN ('pending','connected','syncing','error','disconnected')", name="ck_data_source_status"),
        schema="usf",
    )
    ...
    op.create_table(
        ...
        sa.Column("id", PGUUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("tenant_id", PGUUID(as_uuid=True), sa.ForeignKey("usf.tenant.id", ondelete="CASCADE"), nullable=False),
        sa.Column("source_id", PGUUID(as_uuid=True), sa.ForeignKey("usf.data_source.id"), nullable=False),
        ...
        sa.CheckConstraint("status IN ('pending','running','complete','failed','cancelled')", name="ck_ingestion_job_status"),
        sa.CheckConstraint("mode IN ('full','incremental')", name="ck_ingestion_job_mode"),
        schema="usf",
    )
    ...
    op.create_table(
        ...
        sa.Column("published_by", PGUUID(as_uuid=True), sa.ForeignKey("usf.user.id"), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.UniqueConstraint("tenant_id", "version", name="uq_sdl_version_tenant_version"),
        sa.CheckConstraint("version ~ '^v\\d+$'", name="ck_sdl_version_format"),
        schema="usf",
    )
    op.execute("CREATE UNIQUE INDEX idx_sdl_version_one_active ON usf.sdl_version(tenant_id) WHERE is_active = TRUE")

    op.create_table(
        "audit_log",
        sa.Column("id", PGUUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("tenant_id", PGUUID(as_uuid=True), sa.ForeignKey("usf.tenant.id"), nullable=False),
        sa.Column("user_id", PGUUID(as_uuid=True), sa.ForeignKey("usf.user.id"), nullable=True),
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.CheckConstraint("action IN ('query','ingest','sdl_publish','sdl_validate','login','logout','access_denied','entity_view','audit_export','context_created','source_registered','job_triggered')", name="ck_audit_log_action"),
        sa.CheckConstraint("abac_decision IN ('permit','permit_with_filter','deny')", name="ck_audit_log_abac"),
        schema="usf",
    )
    op.execute("ALTER TABLE usf.audit_log ENABLE ROW LEVEL SECURITY")
    op.execute("CREATE POLICY audit_log_tenant_isolation ON usf.audit_log FOR SELECT USING (tenant_id = current_setting('app.current_tenant_id', true)::UUID)")
    op.execute("CREATE POLICY audit_log_insert_only ON usf.audit_log FOR INSERT WITH CHECK (true)")
    op.create_index("idx_audit_log_tenant_created", "audit_log", ["tenant_id", "created_at"], schema="usf")
    op.create_index("idx_audit_log_query_hash", "audit_log", ["query_hash"], schema="usf", postgresql_where=sa.text("query_hash IS NOT NULL"))
    ...
    op.create_table(
        ...
        sa.Column("completed_at", sa.DateTime(timezone=True), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("NOW()")),
        sa.CheckConstraint("status IN ('pending','running','success','failure','retry','revoked')", name="ck_job_run_status"),
        schema="usf",
    )
    ...
    op.create_table(
        ...
        sa.Column("id", PGUUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("tenant_id", PGUUID(as_uuid=True), sa.ForeignKey("usf.tenant.id", ondelete="CASCADE"), nullable=False),
        sa.Column("sdl_version_id", PGUUID(as_uuid=True), sa.ForeignKey("usf.sdl_version.id"), nullable=True),
        sa.Column("name", sa.Text(), nullable=False),
        ...
        sa.CheckConstraint("name ~ '^[a-z][a-z0-9_-]{0,63}$'", name="ck_context_definition_name"),
        schema="usf",
    )
